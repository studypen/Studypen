...
export const login = async (dispatch: Dispatch<AuthAction>, username: string, password: string): Promise<AxiosResponse | void> => {
  let error = false
  const res = await axios.post<User>('/account/login/', { username, password })
    .catch((err: AxiosError<User>) => { error = true; return err.response })

  if (error)  return res
  else if (res !== undefined) dispatch({ type: actions.LOGIN_SUCCESS, payload: res.data })
}

export const registration = async (dispatch: Dispatch<AuthAction>, userDetail: UserRegistrationDetail): Promise<AxiosResponse | void> => {
  let error = false
  // check return type
